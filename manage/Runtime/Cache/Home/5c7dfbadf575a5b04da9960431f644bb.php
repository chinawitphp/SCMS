	...
	<div class="col-md-9">
		<div class="panel panel-default">
			<div class="panel-heading">
				<h3 class="panel-title">海员证——详细</h3>
			</div>
			<div class="panel-body">
				<form role="form" class="form-horizontal" action="" method="post">
					<div class="form-group">
						...
						<div class="col-sm-3">
							<input type="text" name="name" class="form-control pass_change" id="field-2" placeholder="请输入姓名">
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-3">
							<input type="text" name="birth" class="form-control pass_change" id="field-4" placeholder="请输入出生日期" onclick="laydate()">
						</div>
					...
					<div class="form-group-separator"></div>
					<div class="form-group">
						<label class="col-sm-2 control-label" for="field-6">签发日期</label>
						<div class="col-sm-3">
							<input type="text" name="reg_time" class="form-control pass_change" id="field-6" placeholder="请输入签发日期" onclick="laydate()">
						</div>
						<label class="col-sm-2 control-label" for="field-7">有效期至</label>
						<div class="col-sm-3">
							<input type="text" name="end_time" class="form-control pass_change" id="field-7" placeholder="请输入截止日期" onclick="laydate()">
						</div>
					</div>
					<div class="form-group-separator"></div>
					<div class="form-group">
					...
					<select class="form-control input-lg">
						<option>中华人民共和国辽宁海事局</option>
					</select>
					</div>
					</div>
					<div class="form-group-separator"></div>
					<div class="form-group">
						<button type="button" class="btn btn-info btn-single btn-lg">删除</button>
						<button type="button" class="btn btn-info btn-single btn-lg">修改</button>
					</div>
				</form>
			</div>
		</div>
...
<script type="text/javascript">
	 $(document).ready(function(){
		 ...
		 $.ajax({
		 	type:"GET",
		 	url:"http://123.56.236.250/index.php/home/seamenManage/infodetail/?id="+id+"&type=app",
		 	dateType:"json",
		 	success:function(msg){
		 		for (var i=0;i<msg.length;i++){
		 			var holerid=msg[i].holerid;
		 			var idnumber=msg[i].idnumber;
		 			  var year=idnumber.substr(6,4);
		 			  var month=idnumber.substr(10,2);
		 			  var day=idnumber.substr(12,2);
		 			  var sex_id=idnumber.substr(15,2);
		 			var realname=msg[i].realname;
		 			var regtime=msg[i].regtime;
		 			var endtime=msg[i].endtime;
		 			var nation=msg[i].nation;
		 			var signorgid=msg[i].signorgid;
		 			$("input[name='card']").val(idnumber);
		 			...
		 			$("input[name='reg_time']").val(regtime);
